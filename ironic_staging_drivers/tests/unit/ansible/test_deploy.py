...
class TestAnsibleDeploy(db_base.DbTestCase):
    ...
    def test_prepare_cleaning_callback(self, prepare_cleaning_ports_mock,
                                       prepare_ramdisk_mock,
                                       buid_options_mock,
                                       power_action_mock, get_ip_mock):
        with task_manager.acquire(self.context, self.node.uuid) as task:
...
class TestAnsibleVendor(db_base.DbTestCase):
    ...
    def test_heartbeat_clean_wait(self, set_cleaning_step_mock,
        ...
        with task_manager.acquire(self.context, self.node.uuid) as task:
            ...
            set_cleaning_step_mock.assert_called_once_with(task)
